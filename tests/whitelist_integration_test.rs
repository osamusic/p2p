...
async fn test_whitelist_integration() {
    ...
    whitelist.add_peer(&peer1, Some("Node 1".to_string()), None, None).await.unwrap();
    whitelist.add_peer(&peer2, Some("Node 2".to_string()), None, Some(chrono::Utc::now() + chrono::Duration::hours(24))).await.unwrap();
    ...
    whitelist.add_peer(&expired_peer, None, None, Some(chrono::Utc::now() - chrono::Duration::hours(1))).await.unwrap();
...
async fn test_whitelist_persistence() {
    ...
    {
        let whitelist = PeerWhitelist::new(&db_path).unwrap();
        whitelist.add_peer(&peer_id, Some("Persistent Peer".to_string()), None, None).await.unwrap();
    }
...
async fn test_whitelist_with_public_key() {
    ...
    whitelist.add_peer(&peer_id, Some("Node with Key".to_string()), Some(&public_key), None).await.unwrap();
